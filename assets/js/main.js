...
function initLiveTable(wrapper) {
    ...
    function render() {
        ...
        wrapper.dispatchEvent(new CustomEvent("liveTable:render", {
            detail: {
                total,
                currentRows,
            },
...
function initSummaryLists() {
    const tables = document.querySelectorAll("table[data-simple-list]");
    const summaryModal = createSummaryListModal();
    tables.forEach((table) => initSummaryList(table, summaryModal));
}

function createSummaryListModal() {
    let modalEl = document.getElementById("simpleListDetailModal");
    if (!modalEl) {
        modalEl = document.createElement("div");
        modalEl.className = "modal fade modal-se simple-list-modal";
        modalEl.id = "simpleListDetailModal";
        modalEl.tabIndex = -1;
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="modal-se-title-wrap">
                            <span class="modal-se-icon is-info"><i class="fa-solid fa-table-list"></i></span>
                            <div>
                                <div class="simple-list-modal-kicker">Record Details</div>
                                <h2 class="modal-title h5 mb-0" id="simpleListDetailModalTitle">Details</h2>
                            </div>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div id="simpleListDetailModalBody" class="simple-list-modal-grid"></div>
                    </div>
                </div>
            </div>
        `;
    ...
    return {
        element: modalEl,
        title: modalEl.querySelector("#simpleListDetailModalTitle"),
        body: modalEl.querySelector("#simpleListDetailModalBody"),
        instance: typeof bootstrap !== "undefined" ? bootstrap.Modal.getOrCreateInstance(modalEl) : null,
        movedCells: [],
    };
}

function initSummaryList(table, summaryModal) {
    if (!(table instanceof HTMLTableElement) || table.dataset.simpleListBound === "1") {
        return;
    }

    const tbody = table.tBodies[0];
    const thead = table.tHead;
    if (!tbody || !thead || thead.rows.length === 0) {
        return;
    }

    const headerCells = Array.from(thead.rows[thead.rows.length - 1].cells);
    if (headerCells.length <= 1) {
        return;
    }

    if (!summaryModal || !summaryModal.body || !summaryModal.title || !summaryModal.instance) {
        return;
    }

    const visibleCountRaw = parseInt(String(table.getAttribute("data-simple-list-visible") || "3"), 10);
    const visibleCount = Number.isFinite(visibleCountRaw)
        ? Math.max(1, Math.min(visibleCountRaw, headerCells.length))
        : Math.min(3, headerCells.length);
    const titleSelector = String(table.getAttribute("data-simple-list-title-selector") || "").trim();
    ...
    headerCells.forEach((cell, index) => {
        if (index >= visibleCount) {
            cell.classList.add("simple-list-detail-col");
        }
    });

    const getBodyRows = () => Array.from(tbody.rows).filter((row) => !row.classList.contains("simple-list-detail-row"));

    const rowHasHiddenDetails = (row) => {
        const cells = Array.from(row.cells);
        if (cells.length <= visibleCount) {
            return false;
        }
        const firstCell = cells[0];
        if (cells.length === 1 && firstCell && firstCell.colSpan >= headerCells.length) {
            return false;
        }

        return cells.slice(visibleCount).some((cell) => {
            const text = String(cell.textContent || "").replace(/\s+/g, " ").trim();
            return text !== "" || !!cell.querySelector("a, button, input, select, textarea, form");
        });
    };
    ...
    const decorateRows = () => {
        getBodyRows().forEach((row) => {
            ...
            const isSpanningRow = !!singleCell && singleCell.colSpan >= headerCells.length;

            row.classList.remove("simple-list-row", "is-expanded");
            row.removeAttribute("aria-expanded");

            cells.forEach((cell, index) => {
                cell.classList.remove("simple-list-detail-col");
                if (!isSpanningRow && index >= visibleCount) {
                    cell.classList.add("simple-list-detail-col");
                }
            });

            if (isSpanningRow || !rowHasHiddenDetails(row)) {
                return;
            }

            row.classList.add("simple-list-row");
            row.setAttribute("tabindex", row.getAttribute("tabindex") || "0");
            row.setAttribute("aria-expanded", "false");
        });
    };

    const restoreMovedCells = () => {
        const movedCells = Array.isArray(summaryModal.movedCells) ? summaryModal.movedCells : [];
        movedCells.forEach(({ cell, value }) => {
            while (value.firstChild) {
                cell.appendChild(value.firstChild);
            }
        });
        summaryModal.movedCells = [];
        summaryModal.body.replaceChildren();
        getBodyRows().forEach((row) => row.setAttribute("aria-expanded", "false"));
    };

    const buildDetailModalContent = (row) => {
        restoreMovedCells();

        const detailGrid = document.createElement("div");
        detailGrid.className = "simple-list-detail-grid";
        const movedCells = [];

        Array.from(row.cells).slice(visibleCount).forEach((cell, index) => {
            ...
            detailItem.className = "simple-list-detail-item";

            const label = document.createElement("div");
            label.className = "simple-list-detail-label";
            label.textContent = String(headerCells[visibleCount + index]?.textContent || "Details")
                .replace(/\s+/g, " ")
                .trim() || "Details";

            const value = document.createElement("div");
            value.className = "simple-list-detail-value";
            while (cell.firstChild) {
                value.appendChild(cell.firstChild);
            }
            movedCells.push({ cell, value });
        ...
        summaryModal.movedCells = movedCells;
        return detailGrid;
    };

    const openRowModal = (row) => {
        if (!(row instanceof HTMLTableRowElement) || !row.classList.contains("simple-list-row")) {
            return;
        }
        ...
        let primaryText = "";
        if (titleSelector !== "") {
            const titleSource = row.querySelector(titleSelector);
            primaryText = String(titleSource?.textContent || "").replace(/\s+/g, " ").trim();
        }
        if (primaryText === "") {
            primaryText = String(row.cells[0]?.textContent || "").replace(/\s+/g, " ").trim();
        }
        if (primaryText === "") {
            primaryText = "Details";
        }
        summaryModal.title.textContent = primaryText;
        summaryModal.body.replaceChildren(buildDetailModalContent(row));
        summaryModal.instance.show();
    };
    ...
    const liveTableWrapper = table.closest("[data-live-table]");
    if (liveTableWrapper) {
        liveTableWrapper.addEventListener("liveTable:render", () => {
            restoreMovedCells();
            decorateRows();
        });
    }

    summaryModal.element.addEventListener("hidden.bs.modal", restoreMovedCells);
...
function initRealtimePolling() {
    ...
    const pollRealtime = async () => {
        ...
        try {
            ...
            pollTimerId = window.setTimeout(pollRealtime, intervalMs);
        }
    };
