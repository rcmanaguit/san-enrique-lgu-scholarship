...
$searchAnnouncements = static function (mysqli $conn, string $like, int $limit, bool $isAdmin): array {
    ...
    foreach ($rows as $row) {
        ...
        $items[] = [
            ...
            'meta' => $meta,
            'url' => $isAdmin ? '../shared/sms.php#announcements' : '../announcements.php',
        ];
    }
...
$searchRequirements = static function (mysqli $conn, string $like, int $limit): array {
    ...
    foreach ($rows as $row) {
        $items[] = [
            ...
            'meta' => ((int) ($row['is_active'] ?? 0) === 1 ? 'ACTIVE' : 'INACTIVE'),
            'url' => '../admin-only/application-periods.php#requirements',
        ];
    }
...
$searchPeriods = static function (mysqli $conn, string $like, int $limit): array {
    ...
    $stmt = $conn->prepare(
        "SELECT id, period_name, start_date, end_date, is_open, period_status, notes
    ...
    foreach ($rows as $row) {
        ...
        $items[] = [
            ...
            'subtitle' => $range,
            'meta' => strtoupper(application_period_status_label((string) ($row['period_status'] ?? ((int) ($row['is_open'] ?? 0) === 1 ? 'open' : 'closed')))),
